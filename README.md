...
目前專案內建 `Bing` 與 `Google` 兩個翻譯 provider，預設目標語言為繁體中文（`zht`），並支援用 `text -> lang` 的方式快速指定目標語言。
...
6. 選取翻譯結果後即可複製內容；更多操作中還可以複製原文或開啟對應的翻譯網站。
